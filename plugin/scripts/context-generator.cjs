...
`+JSON.stringify(o,null,2):m=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:f,correlationId:h,...l}=n;Object.keys(l).length>0&&(u=` {${Object.entries(l).map(([S,O])=>`${S}=${O}`).join(", ")}}`)}let T=`[${i}] [${a}] [${_}] ${c}${s}${u}${m}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),m=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:m};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},p=new V;var Nt={};function Ot(){return typeof __dirname<"u"?__dirname:(0,b.dirname)((0,_e.fileURLToPath)(Nt.url))}var Rt=Ot();function At(){if(process.env.CLAUDE_MEM_DATA_DIR)return process.env.CLAUDE_MEM_DATA_DIR;let r=(0,b.join)((0,q.homedir)(),".claude-mem"),e=(0,b.join)(r,"settings.json");try{if((0,k.existsSync)(e)){let{readFileSync:t}=require("fs"),s=JSON.parse(t(e,"utf-8")),n=s.env??s;if(n.CLAUDE_MEM_DATA_DIR)return n.CLAUDE_MEM_DATA_DIR}}catch{}return r}var A=At(),I=process.env.CLAUDE_CONFIG_DIR||(0,b.join)((0,q.homedir)(),".claude"),Gt=process.env.CLAUDE_MEM_PLUGIN_ROOT||"",Ht=(0,b.join)(I,"plugins","marketplaces","thedotmack"),Bt=(0,b.join)(A,"archives"),Wt=(0,b.join)(A,"logs"),Yt=(0,b.join)(A,"trash"),Kt=(0,b.join)(A,"backups"),Vt=(0,b.join)(A,"modes"),qt=(0,b.join)(A,"settings.json"),ce=(0,b.join)(A,"claude-mem.db"),Jt=(0,b.join)(A,"vector-db"),Qt=(0,b.join)(A,"observer-sessions"),zt=(0,b.join)(I,"settings.json"),Zt=(0,b.join)(I,"commands"),es=(0,b.join)(I,"CLAUDE.md");function pe(r){(0,k.mkdirSync)(r,{recursive:!0})}function me(){return(0,b.join)(Rt,"..")}var ue=require("crypto");var Ct=3e4;function w(r,e,t){return(0,ue.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function $(r,e,t){let s=t-Ct;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var P=class{db;constructor(e=ce){e!==":memory:"&&pe(A),this.db=new le.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn()}initializeSchema(){this.db.run(`
