...
set -euo pipefail
# ============================================================
# Load OpenRouter API keys from 1Password → claude-mem settings
#
# Usage: ./scripts/load-openrouter-keys.sh
#
# Updates ~/.claude-mem/settings.json with:
#   CLAUDE_MEM_OPENROUTER_API_KEY  (first available key)
#   CLAUDE_MEM_OPENROUTER_API_KEYS (remaining keys, comma-separated)
#
...
# Map env var names to indices (matches 1Password items)
declare -A KEY_MAP=(
    [1]="OPENROUTER_API_KEY"
    [2]="OPENROUTER_API_KEY2"
    [3]="OPENROUTER_API_KEY3"
    [4]="OPENROUTER_API_KEY4"
    [5]="OPENROUTER_API_KEY5"
)
...
if [ ${#keys[@]} -eq 0 ]; then
    echo "❌ No OpenRouter keys found in environment"
    exit 1
fi
...
settings['CLAUDE_MEM_OPENROUTER_API_KEY'] = '$primary'
settings['CLAUDE_MEM_OPENROUTER_API_KEYS'] = '$pool'
